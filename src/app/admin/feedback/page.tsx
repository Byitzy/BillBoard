...
export default function AdminFeedbackPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'in_progress':
        return 'text-orange-600 bg-orange-50 border-orange-200 dark:bg-orange-900/20';
      case 'resolved':
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <div
                        className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-xs font-medium ${getTypeColor(item.type)}`}
                      >
                        <TypeIcon className="w-3 h-3" />
                        {item.type}
                      </div>
                      <div
                        className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 text-xs font-medium ${getStatusColor(item.status)}`}
                      >
