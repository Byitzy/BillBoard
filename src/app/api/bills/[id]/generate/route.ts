...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: bill, error: billErr } = await admin
    .from('bills')
    .select('id, org_id, project_id, vendor_id, title, amount_total, currency, due_date, recurring_rule, installments_total')
    .eq('id', billId)
    .single();
  if (billErr || !bill) return NextResponse.json({ error: billErr?.message || 'bill not found' }, { status: 404 });
  ...
  const upserts = occ.map((o) => ({
    ...
    state: preserveState(stateMap.get(o.sequence))
  }));
...
function preserveState(prev: string | undefined) {
  ...
  if (prev === 'paid' || prev === 'failed' || prev === 'approved' || prev === 'on_hold') return prev;
  return 'scheduled';
}
